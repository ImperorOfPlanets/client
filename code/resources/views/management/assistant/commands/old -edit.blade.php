		...
		<div class='col-6'>
			...
			<div class='form_selectet p-2'>
				...
				<select class="form-select type-selecter" name='type'>
					...
					<option 
						@if(!is_null($p107) && $p107->pivot->value=='text')
							selected
						@endif
						value="text"
					>
					...
					<option 
						@if(!is_null($p107) && $p107->pivot->value=="handler")
							selected
						@endif
						value="handler"
					>
			...
			<div class='form-inserter p-2'>
				<!-- Сюда будет вставляться форма в зависимости от выбранного типа -->
				@if(!is_null($p107) && $p107->pivot->value=='text')
					Текстовый ответ
					<textarea name='answer' data-focusout='change-property' data-field-id="108" 
							  class='form-control' rows="4">{{$command->propertyByID(108)->pivot->value ?? ''}}</textarea>
					<small class="text-muted">Введите текст ответа на команду</small>
				@elseif(!is_null($p107) && $p107->pivot->value=="handler")
					Обработчик команды
					<input type='text' name='answer' data-focusout='change-property' data-field-id="108" 
						   class='form-control' value="{{$command->propertyByID(108)->pivot->value ?? ''}}"
						   placeholder="App\Handlers\Commands\ExampleHandler@handle">
					<small class="text-muted">Формат: ПолноеИмяКласса@имяМетода</small>
					<div class="mt-2">
						<strong>Примеры:</strong>
						<ul class="small">
							<li>App\Handlers\Commands\WeatherHandler@getWeather</li>
							<li>App\Handlers\Commands\UserHandler@getProfile</li>
							<li>App\Handlers\Commands\NewsHandler@getLatestNews</li>
						</ul>
					</div>
				@endif
			</div>
